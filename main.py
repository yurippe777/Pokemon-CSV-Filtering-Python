...
while True:
    ...
    choices = easygui.multchoicebox("Select one or more abilities to filter by:", "Filter by Ability", ['Any'] + abilities)
    if choices is None:  # Handle close window button click
        sys.exit()
        ...
        mask_ability = df.apply(lambda row: any(x in row[['Ability1', 'Ability2', 'Ability_Hidden']].tolist() for x in choices), axis=1)
